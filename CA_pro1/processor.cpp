...
void get_operation(unsigned int instruction[], unsigned int execu_instru, unsigned int data[], unsigned int load_data) {
	...
	FILE* snapshot = fopen("mysnapshot.rpt", "w+");
	FILE* error_file = fopen("myerror_dump.rpt", "w+");
	printf("cycle %d\n", cycle_num);
	...
	while (opcode != 0x3F) {
		...
		if (opcode == 0x3E) {
			...
			if (opcode == 0) {
				...
				if (funct >= 0x20 && funct <= 0x2A && funct != 0x23 && funct != 0x29) {
					...
					if (rd == 0) {
						...
						case 0x20:
							...
							printf("cycle[rd]=%08X,cycle[rs]=%08X,cycle[rt]=%08X\n", cycle[rd], cycle[rs], cycle[rt]);
							//if (cycle[rs] >> 31 == 0xffffffff && cycle[rt] >> 31 == 0xffffffff)
						...
						case 0x22:
							...
							printf("cycle[rd]=%08X,cycle[rs]=%08X,cycle[rt]=%08X\n", cycle[rd], cycle[rs], minus_rt);
							if ((cycle[rs] > 0 && minus_rt > 0 && cycle[rd] <= 0) || (cycle[rs] < 0 && minus_rt < 0 && cycle[rd] >= 0)) {
									printf("In cycle %d: Number Overflow\n", cycle_num);
									fprintf(error_file, "In cycle %d: Number Overflow\n", cycle_num);
							}
							break;
				...
				if ((uint32_t)(immediate + cycle[rs]) / 4 > 255) {
				...
				if (opcode == 0x21 || opcode == 0x25) {
					...
					if ((immediate + cycle[rs]) % 4 != 0) {
						printf("address = %x", immediate + cycle[rs]);
						printf("In cycle %d: Misalignment Error\n", cycle_num);
						fprintf(error_file, "In cycle %d: Misalignment Error\n", cycle_num);
						halt_flag = true;
					}
				...
				int32_t rt_value = cycle[rt];
				immediate = (0x0000ffff & instr);
				printf("rs=%d,rt=%d,immediate=0x%x\n", rs, rt, immediate);
				if (rt == 0) {
					printf("In cycle %d: Write $0 Error\n", cycle_num);
					fprintf(error_file, "In cycle %d: Write $0 Error\n", cycle_num);
				}
